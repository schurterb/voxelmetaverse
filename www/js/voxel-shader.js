function voxel_shader (require, module, exports) {
    ...
    ShaderPlugin.prototype.createAOShader = function() {
        return glShader(this.shell.gl,
            "<hashtag>define GLSLIFY 1nattribute vec4 attrib0;nattribute vec4 attrib1;nnuniform mat4 projection;nuniform mat4 view;nuniform mat4 model;nuniform float tileCount;nnvarying vec3  normal;nvarying vec2  tileCoord;nvarying float tileSize;nvarying vec2  texCoord;nvarying float ambientOcclusion;nnmat4 translate(float x, float y, float z) { // TODO: @import glslify? same as in avatarn    return mat4(1.0, 0.0, 0.0, 0.0,n                0.0, 1.0, 0.0, 0.0,n                0.0, 0.0, 1.0, 0.0,n                  x,   y,   z, 1.0);n}nnvoid main() {n  //Compute positionn  vec3 position = attrib0.xyz;n  n  //Compute ambient occlusionn  ambientOcclusion = attrib0.w / 255.0;n  n  //Extracted packed bits of normal. GLSL 1.0 doesn't support bitfieldExtract or even bitwise operations :(n  int packedNormal = int(attrib1.x);n  int nx = packedNormal / 16;               // xx____n  int ny = packedNormal / 4 - nx * 4;       // __xx__n  int nz = packedNormal - nx * 16 - ny * 4; // ____xxnn  normal = 128.0 - vec3(nx + 127, ny + 127, nz + 127);n  n  //Compute texture coordinaten  texCoord = vec2(dot(position, vec3(normal.y-normal.z, 0, normal.x)),n                  dot(position, vec3(0, -abs(normal.x+normal.z), normal.y)));n  n  //Compute tile coordinaten  tileSize    = exp2(attrib1.y);n  float tx    = (attrib1.z * 256.0 + attrib1.w) / tileCount; // 16-bitn  tileCoord.x = floor(tx);n  tileCoord.y = fract(tx) * tileCount;nn  // Offset to account for air padding (separate from other matrices so they can be reused)n  mat4 shift = translate(-1.0, -1.0, -1.0);nn  gl_Position = projection * view * model * shift * vec4(position, 1.0);n}n",
            "precision highp float;n<hashtag>define GLSLIFY 1nnuniform sampler2D tileMap;nuniform float tileCount;nnvarying vec3  normal;nvarying vec2  tileCoord;nvarying float tileSize;nvarying vec2  texCoord;nvarying float ambientOcclusion;nnvoid main() {nn  vec2 uv      = texCoord;n  vec4 color   = vec4(0,0,0,0);n  float weight = 0.0;nn  vec2 tileOffset = 2.0 * pow(2.0, 4.0) * tileCoord;n  float denom     = 2.0 * pow(2.0, 4.0) * tileCount;nn  for(int dx=0; dx<2; ++dx) {n    for(int dy=0; dy<2; ++dy) {n      vec2 offset = 2.0 * fract(0.5 * (uv + vec2(dx, dy)));n      float w = pow(1.0 - max(abs(offset.x-1.0), abs(offset.y-1.0)), 16.0);n      n      vec2 tc = (tileOffset + tileSize * offset) / denom;n      color  += w * texture2D(tileMap, tc);n      weight += w;n    }n  }n  color /= weight;n  n  if(color.w < 0.5) {n    discard;n  }n  n  float light = ambientOcclusion + max(0.15*dot(normal, vec3(1,1,1)), 0.0);n  n  gl_FragColor = vec4(color.xyz * light, color.w);n}n"
        )
        // return glslify({
        //   vertex: './lib/ao.vsh',
        //   fragment: './lib/ao.fsh'
    ...
    ShaderPlugin.prototype.createCustomModelShader = function() {
        ...
        return glShader(this.shell.gl,
            "<hashtag>define GLSLIFY 1n                        attribute vec3 position;                        attribute vec2 uv;                                                uniform mat4 projection;                        uniform mat4 view;                        uniform mat4 model;                        varying vec2 vUv;                                                void main() {                        gl_Position = projection * view * model * vec4(position, 1.0);                        vUv = uv;                        }",
            "                        precision highp float;n<hashtag>define GLSLIFY 1n                                                uniform sampler2D texture;                        varying vec2 vUv;                                                void main() {                        gl_FragColor = texture2D(texture, vUv);                        }");
    };
}