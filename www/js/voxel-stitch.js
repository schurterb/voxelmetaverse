function voxel_stitch (require, module, exports) {
    'use strict';
    ...
    function StitchPlugin(game, opts) {
        ...
        if (!this.shell) throw new Error('voxel-stitch requires voxel-engine'); // for gl-init
    ...
    StitchPlugin.prototype.updateTextureSideIDs = function() {
        ...
        for (var name in uvs) {
            ...
            var mx = this.atlasSize,
                my = this.atlasSize;
            var sx = uvTopLeft[0] * mx,
                sy = uvTopLeft[1] * my;
            var ex = uvBottomRight[0] * mx,
                ey = uvBottomRight[1] * my;
            var w = ex - sx;
            var h = ey - sy;
